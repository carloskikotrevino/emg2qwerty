...
class RNNCTCModule(pl.LightningModule):
    """A CTC-based module using an RNN encoder (RNN, LSTM, or GRU) for
    predicting keystrokes from EMG spectrograms.

    Uses the same front-end as TDSConvCTCModule (SpectrogramNorm +
    MultiBandRotationInvariantMLP) for fair comparison, but replaces
    the TDS conv encoder with a recurrent encoder.
    """
    ...
    def __init__(
        ...
        # Front-end: spectrogram normalization + per-band MLP
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        x = self.multiband_mlp(x)  # (T, N, bands=2, mlp_features[-1])
        x = self.flatten(x)  # (T, N, num_features)
        x = self.rnn_encoder(x)  # (T, N, rnn_output_size)
    ...
    def _step(
        ...
        # RNN does not reduce temporal dimension, so emission_lengths = input_lengths
        # However, the spectrogram transform may have changed the length relative to
        # raw EMG. The input_lengths here already account for the spectrogram transform,
        # and since RNN preserves temporal length, emission_lengths == input_lengths.
        T_diff = inputs.shape[0] - emissions.shape[0]
        emission_lengths = input_lengths - T_diff
...
class VisionTransformer(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            ...
            EMGWaveletTransformer(in_channels,embed_dim,num_heads,num_layers,num_classes,patch_size,dropout),
        )
        self.out=nn.Sequential(
            nn.Linear(embed_dim, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
